function OUT = OccludedCentroid_Tuning_IT()
% OCCLUDEDCENTROID_TUNING_IT
% Fit signed IT tuning to the centroid of the back/occluded capsule.
%
% The predictor is stimulus-level, not quartet-level, because the
% front/back assignment can change within a quartet. For each site and
% stimulus, the signed response is normalized as:
...
assert(exist('lsqcurvefit', 'file') == 2, ...
    'OccludedCentroid_Tuning_IT requires lsqcurvefit (Optimization Toolbox).');
...
elseif P.Monkey == 2
    ...
    error('OccludedCentroid_Tuning_IT:InvalidMonkey', ...
        'P.Monkey must be 1 (Nilson) or 2 (Figaro).');
end

tallPath = fullfile(cfg.matDir, tallFile);
resp3binPath = fullfile(cfg.matDir, resp3binFile);
outPath = fullfile(cfg.matDir, sprintf('OccludedCentroid_Tuning_IT_weighted_%s.mat', char(monkeySuffix)));
...
if useCached
    fprintf('Loading cached IT occluded-centroid tuning from %s\n', outPath);
    S = load(outPath);
    ...
    centroidXY = [StimulusTable.occludedCentroidX StimulusTable.occludedCentroidY];
    ...
    %% Fit signed 2D occluded-centroid tuning
    ...
    fprintf('Fitting IT occluded-centroid tuning for monkey %s (%d sites, %d stimuli)\n', ...
...
isOccludedCentroidTunedEarly = isfinite(pEarly) & (pEarly < P.sigAlpha);
isOccludedCentroidTunedLate = isfinite(pLate) & (pLate < P.sigAlpha);

fprintf('Usable IT sites for occluded-centroid fitting (early): %d / %d\n', ...
    nnz(isfinite(veEarly)), nIT);
fprintf('Usable IT sites for occluded-centroid fitting (late):  %d / %d\n', ...
    nnz(isfinite(veLate)), nIT);
if any(isfinite(veEarly))
    fprintf('Median occluded-centroid variance explained early: %.2f%%\n', median(veEarly(isfinite(veEarly))));
end
if any(isfinite(veLate))
    fprintf('Median occluded-centroid variance explained late:  %.2f%%\n', median(veLate(isfinite(veLate))));
end
fprintf('Occluded-centroid tuned sites at p < %.3f (early): %d / %d\n', ...
    P.sigAlpha, nnz(isOccludedCentroidTunedEarly), nIT);
fprintf('Occluded-centroid tuned sites at p < %.3f (late):  %d / %d\n', ...
    P.sigAlpha, nnz(isOccludedCentroidTunedLate), nIT);
...
if P.makeSummaryFigures
    ...
    fprintf('Occluded-centroid variance explained clipped for display below %.1f%%: early=%d, late=%d\n', ...
    ...
    ylabel('N sites');
    title(sprintf('IT occluded-centroid tuning variance explained (%s, floor %.0f%%)', ...
    ...
    xlabel('Fitted modulation range over observed occluded centroids (spont. SD units)');
    ylabel('N sites');
    title(sprintf('IT occluded-centroid tuning effect size (%s)', char(monkeySuffix)));
    legend('Early', 'Late');
    grid on;

    figure('Color', 'w');
    scatter([FitLate.muX], [FitLate.muY], 28, veLate, 'filled');
    xlabel('Preferred occluded-centroid x');
    ylabel('Preferred occluded-centroid y');
    title(sprintf('IT late preferred occluded-centroid position (color = %% variance explained, %s)', ...
        char(monkeySuffix)));
    colorbar;
...
if P.makeExampleFigure
    make_example_fit_figure(centroidXY, signedStimEarly, FitEarly, ...
        sprintf('IT occluded-centroid fits early (%s)', char(monkeySuffix)), P.nExampleSites, RFrange);
    make_example_fit_figure(centroidXY, signedStimLate, FitLate, ...
        sprintf('IT occluded-centroid fits late (%s)', char(monkeySuffix)), P.nExampleSites, RFrange);
end

%% Top-site tables
Tearly = build_fit_table(FitEarly, RFrange, 'r2TrainPct');
Tlate = build_fit_table(FitLate, RFrange, 'r2TrainPct');
Tearly.isOccludedCentroidTuned = isOccludedCentroidTunedEarly(Tearly.localSite);
Tlate.isOccludedCentroidTuned = isOccludedCentroidTunedLate(Tlate.localSite);

disp('Top IT occluded-centroid sites by variance explained (early):');
disp(Tearly(1:min(10,height(Tearly)), :));
disp('Top IT occluded-centroid sites by variance explained (late):');
...
OUT.isOccludedCentroidTunedEarly = isOccludedCentroidTunedEarly;
OUT.isOccludedCentroidTunedLate = isOccludedCentroidTunedLate;
OUT.TableEarly = Tearly;
OUT.TableLate = Tlate;

if needsSave && P.saveResult
    save(outPath, 'OUT', '-v7.3');
    fprintf('Saved IT occluded-centroid tuning to %s\n', outPath);
end
end
...
for stimIdx = 1:nStim
    [cueXY(stimIdx,:), tBackXY(stimIdx,:), centroidXY(stimIdx,:)] = ...
        occluded_centroid_from_stim(ALLCOORDS, stimIdx);
end

StimulusTable = table(stimNum, cueXY(:,1), cueXY(:,2), tBackXY(:,1), tBackXY(:,2), ...
    centroidXY(:,1), centroidXY(:,2), 'VariableNames', ...
    {'stimNum','cueX','cueY','tBackX','tBackY','occludedCentroidX','occludedCentroidY'});
end

function [cue, tBack, centroid] = occluded_centroid_from_stim(ALLCOORDS, stimNum)
f = sprintf('stim_%d', stimNum);
cue = double(ALLCOORDS.(f).s(:))';
tBack = double(ALLCOORDS.(f).t_back(:))';
centroid = 0.5 * (cue + tBack);
end
...
for i = 1:nShow
    ...
    xlabel(ax, 'Occluded centroid x');
    if i == 1
        ylabel(ax, 'Occluded centroid y');
    else
        ylabel(ax, '');
    end
