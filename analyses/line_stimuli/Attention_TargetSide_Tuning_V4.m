function OUT = Attention_TargetSide_Tuning_V4()
% ATTENTION_TARGETSIDE_TUNING_V4
% Fit V4 tuning to the screen direction of the attended capsule relative to
...
elseif P.Monkey == 2
    ...
    error('Attention_TargetSide_Tuning_V4:InvalidMonkey', ...
        'P.Monkey must be 1 (Nilson) or 2 (Figaro).');
end

tallPath = fullfile(cfg.matDir, tallFile);
resp3binPath = fullfile(cfg.matDir, resp3binFile);
outPath = fullfile(cfg.matDir, sprintf('Attention_TargetSide_Tuning_V4_directiondelta_%s.mat', char(monkeySuffix)));
...
if useCached
    fprintf('Loading cached V4 target-side tuning from %s\n', outPath);
    S = load(outPath);
    ...
    RFrange = OUT.RFrange(:);
    nV4 = numel(RFrange);
    if isfield(OUT, 'bestSNR')
        bestSNR = OUT.bestSNR(:);
    else
        bestSNR = nan(nV4, 1);
    end
    if isfield(OUT, 'keepVisual')
        keepVisual = OUT.keepVisual(:);
    else
        keepVisual = isfinite(bestSNR) & (bestSNR > P.VisualSNRthr);
    end
else
    if hasSessionExclusions
        fprintf(['Session exclusions are active for monkey %s; recomputing cached V4 ' ...
                 'target-side tuning.\n'], char(monkeySuffix));
    end
    %% Load geometry
    Sgeo = load(tallPath);
    ...
    assert(all(stimNumsSorted(:).' == 1:numel(Tall_V4)), ...
        'Tall_V4.stimNum must cover 1..%d exactly.', numel(Tall_V4));
    Tall_V4 = Tall_V4(ordStim);
    nStim = numel(Tall_V4);
    ...
    %% Load 3-bin responses and localize to V4 rows
    ...
    assert(size(R3.meanAct,1) == nV4, 'Localized V4 response rows do not match Tall_V4.');
    assert(size(R3.meanAct,2) == nStim, 'Localized V4 response stimuli do not match Tall_V4.');
    ...
    SNRmat = nan(nV4, numel(snrFields));
    for k = 1:numel(snrFields)
        SNRmat(:,k) = double(SNR.(snrFields{k})(siteRows));
    end
    bestSNR = max(SNRmat, [], 2, 'omitnan');
    keepVisual = isfinite(bestSNR) & (bestSNR > P.VisualSNRthr);
    fprintf('Keeping %d / %d V4 sites for target-direction fits (bestSNR > %.2f)\n', ...
        nnz(keepVisual), nV4, P.VisualSNRthr);
    ...
    FitEarly = init_fit_struct(nV4, fields, "direction");
    FitLate = init_fit_struct(nV4, fields, "direction");

    fprintf('Fitting V4 target-side attention-direction tuning for monkey %s (%d sites, %d quartets)\n', ...
        char(monkeySuffix), nV4, nQuartets);
    tFit = tic;
    ...
    for iSite = 1:nV4
        ...
        if P.progressEvery > 0 && (iSite == 1 || mod(iSite, P.progressEvery) == 0 || iSite == nV4)
            elapsedSec = toc(tFit);
            rate = iSite / max(elapsedSec, eps);
            etaSec = (nV4 - iSite) / max(rate, eps);
            fprintf('  Site %d / %d | elapsed %.1fs | ETA %.1fs\n', ...
                iSite, nV4, elapsedSec, etaSec);
        end
    end
end
...
fprintf('Usable V4 sites for target-direction fitting (early): %d / %d\n', ...
    nnz(isfinite(veEarly)), numel(RFrange));
fprintf('Usable V4 sites for target-direction fitting (late):  %d / %d\n', ...
    nnz(isfinite(veLate)), numel(RFrange));
...
if P.makeAngleReferenceFigure
    make_target_side_reference_figure(ALLCOORDS, RTAB384, QuartetTable, ...
        sprintf('V4 target-side angle reference (%s)', char(monkeySuffix)));
end
...
if P.makeExampleFigures
    ...
    make_example_angle_fit_figure(thetaDeg, deltaQuartetEarly, FitEarly, ...
        sprintf('V4 target-direction fits early (%s)', char(monkeySuffix)), siteSelEarly, 1);
    make_example_angle_fit_figure(thetaDeg, deltaQuartetLate, FitLate, ...
        sprintf('V4 target-direction fits late (%s)', char(monkeySuffix)), siteSelLate, 1);
end
...
disp('Top V4 target-side direction sites by variance explained (late):');
...
if needsSave && P.saveResult
    ...
    fprintf('Saved V4 target-side tuning to %s\n', outPath);
end
end
...
v = tFig - tBack;
if norm(v) <= 0
    error('Attention_TargetSide_Tuning_V4:DegenerateTargetSide', ...
