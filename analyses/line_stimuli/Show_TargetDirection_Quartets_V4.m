function OUT = Show_TargetDirection_Quartets_V4()
% SHOW_TARGETDIRECTION_QUARTETS_V4
% Illustrate a few V4 sites with strong target-direction tuning by showing
...
elseif P.Monkey == 2
    ...
    error('Show_TargetDirection_Quartets_V4:InvalidMonkey', ...
...
distCtrlPath = fullfile(cfg.matDir, sprintf('Attention_TargetSide_DistanceControl_V4_%s.mat', char(monkeySuffix)));
...
if ~hasSessionExclusions
    ...
    if strcmpi(P.siteSelectionMode, 'cardinal_plus_dirnotdist')
        assert(exist(distCtrlPath, 'file') == 2, ...
            'Missing %s. Run Attention_TargetSide_DistanceControl_V4 first.', distCtrlPath);
    end
end
...
Stall = load(tallPath, 'ALLCOORDS', 'RTAB384', 'RFrange', 'Tall_V4');
assert(isfield(Stall, 'ALLCOORDS') && isfield(Stall, 'RTAB384') && isfield(Stall, 'RFrange') && ...
    isfield(Stall, 'Tall_V4'), '%s must contain ALLCOORDS, RTAB384, RFrange, and Tall_V4.', tallPath);
...
siteIdx = find(keep);
assert(~isempty(siteIdx), 'No V4 sites pass the selected late target-direction criterion.');
...
elseif strcmpi(P.siteSelectionMode, 'cardinal_plus_dirnotdist')
    ...
    if hasSessionExclusions
        D = Attention_TargetSide_DistanceControl_V4();
    else
...
for ii = 1:numel(siteSel)
    ...
    for jj = 1:numel(qSel)
        ...
        Site(ii).rfXpx(jj) = Tall_V4(stimShow).T.x_px(s);
        Site(ii).rfYpx(jj) = Tall_V4(stimShow).T.y_px(s);
    end
...
fig = figure('Color', 'w', 'Name', sprintf('V4 target-direction quartets site %d', S.globalSiteInR), ...
...
for j = 1:nQ
    ...
    'String', sprintf('V4 site %d (%s) | %s | late VE %.1f%% | pref %.0f%c', ...
