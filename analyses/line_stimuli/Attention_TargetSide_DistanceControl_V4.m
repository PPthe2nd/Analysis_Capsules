function OUT = Attention_TargetSide_DistanceControl_V4()
% ATTENTION_TARGETSIDE_DISTANCECONTROL_V4
% Test whether the V4 target-direction effect survives after accounting for
...
elseif P.Monkey == 2
    ...
    error('Attention_TargetSide_DistanceControl_V4:InvalidMonkey', ...
        'P.Monkey must be 1 (Nilson) or 2 (Figaro).');
end

basePath = fullfile(cfg.matDir, sprintf('Attention_TargetSide_Tuning_V4_directiondelta_%s.mat', char(monkeySuffix)));
tallPath = fullfile(cfg.matDir, tallFile);
outPath = fullfile(cfg.matDir, sprintf('Attention_TargetSide_DistanceControl_V4_%s.mat', char(monkeySuffix)));
hasSessionExclusions = ~isempty(site_session_exclusions(monkeySuffix));

assert(exist(tallPath, 'file') == 2, ...
    'Missing %s. Run Line_Stimuli_V4 first.', tallPath);

useCached = exist(outPath, 'file') == 2 && ~P.forceRefit && ~hasSessionExclusions;
if useCached
    fprintf('Loading cached V4 distance-control analysis from %s\n', outPath);
    S = load(outPath, 'OUT');
...
if hasSessionExclusions
    fprintf(['Session exclusions are active for monkey %s; refreshing V4 target-side ' ...
             'fits before distance-control regression.\n'], char(monkeySuffix));
    BASE = Attention_TargetSide_Tuning_V4();
else
    assert(exist(basePath, 'file') == 2, ...
        'Missing %s. Run Attention_TargetSide_Tuning_V4 first.', basePath);
    Sbase = load(basePath, 'OUT');
    assert(isfield(Sbase, 'OUT') && isstruct(Sbase.OUT), '%s must contain OUT.', basePath);
    BASE = Sbase.OUT;
end
requiredBase = {'QuartetTable','deltaQuartetLate','varQuartetLate','FitLate','RFrange','keepVisual','bestSNR'};
...
nV4 = numel(RFrange);
keepVisual = logical(BASE.keepVisual(:));
bestSNR = double(BASE.bestSNR(:));
nQuartets = height(QuartetTable);

%% Load geometry
Stall = load(tallPath, 'Tall_V4', 'ALLCOORDS', 'RTAB384');
assert(isfield(Stall, 'Tall_V4') && isstruct(Stall.Tall_V4), '%s must contain Tall_V4.', tallPath);
assert(isfield(Stall, 'ALLCOORDS') && isfield(Stall, 'RTAB384'), ...
    '%s must contain ALLCOORDS and RTAB384.', tallPath);
Tall_V4 = Stall.Tall_V4;
ALLCOORDS = Stall.ALLCOORDS;
RTAB384 = Stall.RTAB384;

%% Distances from RF to target and distractor arms for every stimulus
[distTargetPx, distDistractorPx] = compute_arm_distances_all_stim(ALLCOORDS, RTAB384, Tall_V4, nV4);

%% Quartet-wise distance predictors aligned to DirA versus DirB
targetAdvPx = nan(nV4, nQuartets);
meanDistPx = nan(nV4, nQuartets);
...
RegressionLate = init_regression_struct(nV4);
for iSite = 1:nV4
    if ~keepVisual(iSite)
        continue;
    end
...
fprintf('Included V4 sites (bestSNR > %.2f): %d / %d\n', BASE.P.VisualSNRthr, nnz(keepVisual), nV4);
...
if P.saveResult
    ...
    fprintf('Saved V4 distance-control analysis to %s\n', outPath);
end
...
function [distTargetPx, distDistractorPx] = compute_arm_distances_all_stim(ALLCOORDS, RTAB384, Tall_V4, nV4)
nStim = numel(Tall_V4);
distTargetPx = nan(nV4, nStim);
distDistractorPx = nan(nV4, nStim);

fprintf('Computing RF-to-arm distances for %d V4 sites across %d stimuli...\n', nV4, nStim);
for stimNum = 1:nStim
    ...
    T = Tall_V4(stimNum).T;
    assert(height(T) >= nV4, 'Tall_V4(%d).T has fewer than %d V4 rows.', stimNum, nV4);
    x = round(double(T.x_px(1:nV4)));
    y = round(double(T.y_px(1:nV4)));
    H = size(dTargetMap, 1);
    W = size(dTargetMap, 2);
    inB = x >= 1 & x <= W & y >= 1 & y <= H;
    if ismember('inBounds', string(T.Properties.VariableNames))
        inB = inB & logical(T.inBounds(1:nV4));
    end
...
scatter_unique_r2(OUT.uniqueR2Dir, OUT.uniqueR2Dist, OUT.ctrlSig, OUT.distGivenDirSig, ...
    sprintf('V4 Unique Additive Variance (%s)', char(OUT.monkeySuffix)));
...
ylabel('N sites');
title(sprintf('V4 late regression counts (%s)', char(OUT.monkeySuffix)));
grid on;
