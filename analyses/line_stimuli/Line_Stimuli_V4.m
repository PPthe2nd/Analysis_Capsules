% Build stimulus-aligned RF geometry tables for V4 sites in the line task.
% Mirrors the V1 setup in Line_Stimuli.m, but keeps V4 as a separate entry
% point so the existing V1 workflow stays unchanged.
...
elseif Monkey == 2
    ...
    error('Line_Stimuli_V4:InvalidMonkey', ...
        'Monkey must be 1 (Nilson) or 2 (Figaro).');
end

outFile = fullfile(cfg.matDir, sprintf('Tall_V4_lines_%s.mat', char(monkeySuffix)));
...
RFrange = V4;
if isempty(RFrange)
    error('Line_Stimuli_V4:NoV4Sites', ...
        'No V4 site indices are available for monkey %d.', Monkey);
end

x_rf = x(RFrange);
y_rf = y(RFrange);

fprintf('Using %d V4 sites for monkey %s.\n', numel(RFrange), MonkeyName);
...
if MakeExamplePlot
    ...
    title(sprintf('V4 object-assigned RF centers on stim %d', stimNum));
end

% Build the full per-stimulus geometry table for all 384 line-task stimuli.
if skipBuild
    fprintf('Skipping full V4 geometry build because output already exists:\n%s\n', outFile);
    fprintf('Set ForceRebuild = true to rebuild Tall_V4.\n');
    return;
end

Tall_V4 = build_all_stim_tables(ALLCOORDS, RTAB384, x_rf, y_rf);

save(outFile, 'Tall_V4', 'ALLCOORDS', 'RTAB384', 'Monkey', 'MonkeyName', ...
    'RFrange', 'TabFile', '-v7.3');

fprintf('Saved V4 geometry table to %s\n', outFile);
