function OUT = Attention_ColorPref_Line_Stimuli_V4()
% ATTENTION_COLORPREF_LINE_STIMULI_V4
% Time-resolved V4 responses for the 4-way color-by-attention interaction:
...
%
% Preferred color is defined by the sign of ColorTune.<window>.colorIndex:
%   > 0 => yellow preferred, < 0 => purple preferred.
%
% The interaction test mirrors Attention_ColorPref_Line_Stimuli.m:
...
P.colorTuneFileN = 'ColorTune_balanced_V4_N.mat';
P.colorTuneFileF = 'ColorTune_balanced_V4_F.mat';
...
elseif P.Monkey == 2
    ...
    error('Attention_ColorPref_Line_Stimuli_V4:InvalidMonkey', ...
...
assert(exist(respPath, 'file') == 2, ...
    'Missing %s. Create the high-resolution V4 response summary first.', respPath);
assert(exist(resp3binPath, 'file') == 2, ...
    'Missing %s. Create the 3-bin V4 response summary first.', resp3binPath);
...
if exist(colorTunePath, 'file') == 2
    ...
    assert(~isempty(keepSiteIdx3), ...
        'No V4 sites passed the SNR threshold %.2f during ColorTune construction.', P.colorTuneSNRthr);
...
if exist(out3File, 'file') == 2
    ...
    optsTD = struct('v1Sites', v4Sites, 'timeIdx', P.attentionTimeIdx, ...
        'excludeOverlap', P.excludeOverlap, 'verbose', true, 'epsDen', 1e-6);
    OUT3 = attention_modulation_V1_3bin(R3, Tall_V4, SNR, optsTD);
    OUT = OUT3; %#ok<NASGU>
...
assert(nCh == nV4, 'Expected %d localized V4 rows in R_resp.meanAct, got %d.', nV4, nCh);
...
assert(numel(OUT3.pValueTD) >= max(v4Sites), 'OUT3.pValueTD smaller than max(v4Sites).');
assert(numel(ColorTune.(P.colorTuneWindow).p) >= max(v4Sites), ...
    'ColorTune.%s.p smaller than max(v4Sites).', P.colorTuneWindow);
assert(numel(ColorTune.(P.colorTuneWindow).colorIndex) >= max(v4Sites), ...
    'ColorTune.%s.colorIndex smaller than max(v4Sites).', P.colorTuneWindow);

tCenters = mean(double(R_resp.timeWindows), 2);

%% Site selection: intersection of attention and color significance
pAtt = double(OUT3.pValueTD(v4Sites));
pCol = double(ColorTune.(P.colorTuneWindow).p(v4Sites));
ci = double(ColorTune.(P.colorTuneWindow).colorIndex(v4Sites));
...
siteLocalAll = find(isBoth);      % indices in 1..nV4
siteGlobalAll = RFrange(siteLocalAll);

fprintf('V4 attention-significant sites: %d / %d (p < %.3f)\n', nnz(isAttSig), nV4, P.pAttentionThresh);
fprintf('V4 color-significant sites: %d / %d (p < %.3f, window=%s)\n', ...
    nnz(isColorSig), nV4, P.pColorThresh, P.colorTuneWindow);
fprintf('V4 intersection sites (attention & color): %d / %d\n', numel(siteLocalAll), nV4);

%% Sort Tall_V4 by stimulus number and build per-site/stim masks
stimNums = arrayfun(@(x) x.stimNum, Tall_V4(:));
[stimNumsSorted, ord] = sort(stimNums(:));
assert(numel(stimNumsSorted) == nStim && all(stimNumsSorted(:).' == 1:nStim), ...
    'Tall_V4.stimNum must cover 1..%d exactly.', nStim);
TallSorted = Tall_V4(ord);

isTarget = false(nV4, nStim);
isDistr  = false(nV4, nStim);
isBG     = false(nV4, nStim);
isY      = false(nV4, nStim);
isP      = false(nV4, nStim);
isOV     = false(nV4, nStim);

for s = 1:nStim
    assert(isfield(TallSorted(s), 'T') && istable(TallSorted(s).T), ...
        'Tall_V4(%d).T missing or not a table.', s);
    T = TallSorted(s).T;
    assert(height(T) >= max(v4Sites), ...
        'Tall_V4(%d).T has %d rows; need at least %d.', s, height(T), max(v4Sites));
    vn = string(T.Properties.VariableNames);
    assert(ismember("assignment", vn) && ismember("center_color", vn), ...
        'Tall_V4(%d).T must contain assignment and center_color.', s);

    asg = toCellStrLocal(T.assignment(v4Sites));
    col = toCellStrLocal(T.center_color(v4Sites));
...
if P.normalizeResponses
    ...
    bAll = double(SNR.muSpont(v4Sites));
    topMat = [double(SNR.muYellowEarly(v4Sites)), ...
              double(SNR.muYellowLate(v4Sites)), ...
              double(SNR.muPurpleEarly(v4Sites)), ...
              double(SNR.muPurpleLate(v4Sites))];
...
elseif ismatrix(nTrialsRaw) && (size(nTrialsRaw,2) == nStim)
    perSiteTrials = true;
    nTrialsByStim = [];
    assert(size(nTrialsRaw,1) >= max(v4Sites), ...
        'R_resp.nTrials has %d rows; need at least %d.', size(nTrialsRaw,1), max(v4Sites));
else
...
for k = 1:nSiteCand
    ...
    assert(size(resp,1) == nStim && size(resp,2) == nBins, ...
        'Unexpected response shape at localized V4 row %d.', ch);
...
fprintf('V4 sites with all 4 conditions present: %d / %d intersection sites\n', ...
...
fprintf('\nV4 interaction test (additive vs multiplicative):\n');
...
if P.plotFigure
    ...
    title(ax, sprintf('V4 Best/Worst color x Attention (%s, N=%d sites)', ...
        char(monkeySuffix), numel(siteLocal)));
...
if P.plotInteractionFigure
    ...
    title(ax1, 'V4 attention effect by color level');
...
outFile = fullfile(cfg.resultsDir, ...
    sprintf('attention_colorpref_timeseries_V4_%s_%s.mat', char(monkeySuffix), resultTag));

if P.saveResult
    save(outFile, 'OUT', '-v7.3');
    fprintf('Saved V4 color x attention time-course result to: %s\n', outFile);
end
