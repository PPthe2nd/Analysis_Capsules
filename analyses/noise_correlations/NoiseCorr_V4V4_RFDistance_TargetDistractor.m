function OUT = NoiseCorr_V4V4_RFDistance_TargetDistractor(Monkey, Puser)
% NOISECORR_V4V4_RFDISTANCE_TARGETDISTRACTOR
% Plot V4-V4 noise correlation versus RF distance for target/distractor/
% background pair conditions. RF distance uses the V4 RF center
% coordinates in pixels from Tall_V4; condition labels also use Tall_V4.
...
outPath = fullfile(cfg.resultsDir, sprintf('NoiseCorr_V4V4_RFDistance_TargetDistractor_%s_%s.mat', char(monkeySuffix), char(P.cacheTag)));
...
if P.useCache && exist(outPath, 'file') == 2
    ...
    if isfield(S, 'OUT') && local_cache_matches(S.OUT, cacheParams) && ...
        ...
        OUT.P = P;
        if P.verbose
            fprintf('Loaded cached V4-V4 RF-distance noise-correlation analysis from %s\n', outPath);
        end
        if P.plotFigure
...
pairBlock = local_load_v4v4_pair_block(Master, cfg, monkeySuffix, P.cacheTag);
...
assignCode = local_build_assign_code(Tall_V4, SiteTableV4.areaLocalSite);
xPxAll = double(Tall_V4(1).T.x_px(:));
yPxAll = double(Tall_V4(1).T.y_px(:));
xPx = xPxAll(double(SiteTableV4.areaLocalSite));
yPx = yPxAll(double(SiteTableV4.areaLocalSite));

siteIdxToV4Row = nan(height(Master.SiteTable), 1);
siteIdxToV4Row(siteRowsV4) = 1:height(SiteTableV4);
pairSite1V4 = siteIdxToV4Row(double(PairTable.site1Idx));
pairSite2V4 = siteIdxToV4Row(double(PairTable.site2Idx));
assert(all(isfinite(pairSite1V4)) && all(isfinite(pairSite2V4)), ...
    'V4-V4 pair block contains non-V4 sites.');

nPairs = height(PairTable);
rfDistancePx = hypot(xPx(pairSite1V4) - xPx(pairSite2V4), ...
                     yPx(pairSite1V4) - yPx(pairSite2V4));
...
if P.saveResult
    ...
    if P.verbose
        fprintf('Saved V4-V4 RF-distance analysis to %s\n', outPath);
    end
end
...
function pairBlock = local_load_v4v4_pair_block(Master, cfg, monkeySuffix, cacheTag)
pairBlock = struct();
pairBlock.pairFile = "";

if isfield(Master, 'PairClassTable') && istable(Master.PairClassTable)
    idx = find(Master.PairClassTable.pairClass == "V4-V4", 1, 'first');
    assert(~isempty(idx), 'Master cache does not contain V4-V4 pair metadata.');
...
assert(isfield(Master, 'sumXY') && isfield(Master, 'PairTable') && isfield(Master, 'PairClassTable'), ...
    'Master cache does not contain V4-V4 pair data and no pair file is available.');
idx = find(Master.PairClassTable.pairClass == "V4-V4", 1, 'first');
assert(~isempty(idx), 'Master cache does not contain V4-V4 pair metadata.');
s = Master.PairClassTable.startIdx(idx);
e = Master.PairClassTable.endIdx(idx);
assert(isfinite(s) && isfinite(e), 'Invalid V4-V4 pair range in master cache.');
...
figure('Color', 'w', 'Name', sprintf('V4-V4 late-minus-pre summary (%s)', char(OUT.monkeySuffix)));
ax = axes();
hold(ax, 'on');
...
colors = [ ...
    ...
    0.45 0.45 0.45; ... % B-T
    0.72 0.72 0.72];    % B-D
figure('Color', 'w', 'Name', sprintf('V4-V4 noise corr vs RF distance (%s)', char(OUT.monkeySuffix)));
...
for wSel = 1:numel(OUT.windowIdx)
    ...
    title(ax1, sprintf('V4-V4 noise correlation | %d-%d ms | min trials = %d', ...
