function OUT = PSTH_targetColor_V4(Monkey, Opts)
% PSTH_TARGETCOLOR_V4
% Balanced V4 time course for best vs worst target-capsule color,
...
elseif Monkey == 2
    ...
    error('PSTH_targetColor_V4:InvalidMonkey', ...
...
[TallSorted, targetColorByStim] = build_target_color_labels_local(Tall_V4, RTAB384, nStim);
...
fprintf('V4 target-color PSTH (%s, %s window)\n', char(monkeySuffix), Opts.Window);
fprintf('Base site pool: %s\n', baseLabel);
fprintf('Visual V4 sites in base pool: %d / %d\n', nnz(keepBase), nV4);
fprintf('Valid paired target-color metric in base pool: %d / %d\n', nnz(validTarget), nV4);
fprintf('Target-color pairedP < 0.05 in base pool: %d / %d\n', ...
    nnz(keepBase & isfinite(pPaired) & (pPaired < 0.05)), nV4);
fprintf('Using selection: %s\n', selectionLabel);
fprintf('Selected %d / %d V4 sites for PSTH\n', nnz(keep), nV4);

assert(any(keep), 'No V4 sites passed the requested target-color selection criteria.');
...
elseif ismatrix(nTrialsRaw) && (size(nTrialsRaw, 2) == nStim)
    perSiteTrials = true;
    nTrialsByStim = [];
    assert(size(nTrialsRaw, 1) >= nV4, ...
        'R_resp.nTrials has %d rows; need at least %d.', size(nTrialsRaw, 1), nV4);
else
...
OUT = finalize_output_local(muBest, muWorst, muTargetYellow, muTargetPurple, ...
    ...
    prefIsTargetYellowSel, tCenters, selectionLabel, monkeySuffix, 'V4', Opts);
OUT.bestSNR = bestSNR;
